...
fun CategoriesScreen(
    onCategoryClick: (String) -> Unit, viewModel: CategoriesViewModel = hiltViewModel()
) {
    val state by viewModel.uiState.collectAsStateWithLifecycle()
    CategoriesContent(state = state, onRetry = viewModel::refresh, onCategoryClick = onCategoryClick)
}

@Composable
private fun CategoriesContent(
    state: CategoriesUiState, onRetry: () -> Unit, onCategoryClick: (String) -> Unit
) {
            ...
            state.error != null -> ErrorView(state.error, onRetry, Modifier.fillMaxSize())
                ...
                items(state.data) { cat ->
                    Card(
                        ...
                        Text(
                            text = cat.name.ifBlank { cat.slug }, modifier = Modifier.padding(16.dp), color = Color(0xFF0D47A1)
                        )
                    }
                }
            }
